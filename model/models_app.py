...
print "Test data size is {}".format(training_data_raw.count())
...
labels_and_preds = training_data.map(lambda p: (float(logit_model.predict(p.features)), p.label  ))
...
# compute accuracy
training_accuracy = labels_and_preds.filter(lambda (v,p): v == p).count() / float(training_data.count())

print "Training accuracy {}".format(round(training_accuracy, 4))
###############/Logistic Regression #########################
...
predictions = dt_model.predict(training_data.map(lambda p: p.features))
labels_and_preds = training_data.map(lambda p: p.label).zip(predictions)
...
print "Prediction made in {} seconds. Test accuracy is {}".format(round(tt,3), round(training_accuracy,4))
#################### /Decision Tree ##########################
